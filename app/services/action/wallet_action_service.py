...
from app.data.models import Transaction, TransactionLog, Wallet, WalletOperation
...
def send_money(form:SendMoneyForm, user_id:int, session:Session) -> ActionResult:
    ...
    # check balance
    if sender_current_balance < form.amount:
    ...
    sender_wallet.current_balance = sender_current_balance - form.amount

    # update receiver last balance
    receiver_wallet.last_balance = receiver_current_balance
    # update receiver current balance
    receiver_wallet.current_balance = receiver_current_balance + form.amount
